...
function writeRichText(rte, html, width, height, buttons, readOnly) {
	...
	//		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'redo.gif" width="25" height="24" alt="Redo" title="Redo" onClick="rteCommand(\'' + rte + '\', \'redo\')"></td>');
		...
		if (!readOnly) document.writeln('<br /><input type="checkbox" id="chkSrc' + rte + '" onclick="toggleHTMLSrc(\'' + rte + '\');" />&nbsp;Ver código fonte');
...
function rteCommand(rte, command, option) {
	...
	try {
		oRTE.focus();
	  	ret = oRTE.document.execCommand(command, false, option);
		oRTE.focus();
	} catch (e) {
//		alert(e);
//		setTimeout("rteCommand('" + rte + "', '" + command + "', '" + option + "');", 10);
...
function addImage(rte) {
	rteAux = rte;
	window.open('/AreaRestrita/Includes/rte/listaImagens.cfm','winListaImg',"height=400,width=450,status=0,scrollbars=1,resizable=1,screenX=0,screenY=0,left=0,top=0")
}
function addImage_f4w(imagePath){ //function to add image
	if ((imagePath != null) && (imagePath != "")) {
		rteCommand(rteAux, 'InsertImage', imagePath);
	}
}
